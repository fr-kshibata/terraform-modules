resource "aws_launch_configuration" "kube_master" {
  ...
  security_groups             = ["${aws_security_group.kube_master.id}", "${var.etcd_security_group}"]
...
resource "aws_autoscaling_group" "kube_master" {
  ...
  load_balancers            = ["${aws_elb.kube_master.name}"]
